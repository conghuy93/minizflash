...
// Cloudflare Worker Proxy - firmware URLs hidden server-side
const WORKER_PROXY_URL = 'https://firmware-proxy.minizjp.workers.dev/';
...
class ESPWebFlasher {
    ...
    async selectGithubFirmware(card) {
        ...
        try {
            // Download via Cloudflare Worker proxy - GitHub URLs completely hidden
            console.log('🔒 Downloading firmware via secure proxy...');
            const proxyResponse = await fetch(WORKER_PROXY_URL, {
                method: 'POST',
            ...
            this.selectedGithubUrl = `Firmware ${firmwareId} (via proxy)`;
